...
```{r pt2_hex_map_yr_bldg_weighted, message=FALSE, warning=FALSE}
...
```{r pt2_hex_map_yr_animate_bldg_weighted, eval=FALSE, message=FALSE, warning=FALSE}
...
# Part 3: Time Series Line Graphs by Annexation Status and Land Sale Year
...
```{r pt3_data, results=FALSE, message=FALSE, warning=FALSE}
pt3_data = left_join(pt2_hexagons_hom_ct_yr_ALL, hex_bldgs, by = c("GRID_ID", "year")) %>%
...
This is done by calculating the centroids of the hexagons, and spatially assigning those centroids to the surrounding area.

```{r pt3_hex_annexation_status, results=FALSE, message=FALSE, warning=FALSE}
...
```{r pt3_annexation_map, message=FALSE, warning=FALSE}
tmap_mode("view")
...
```{r pt3_annexed_aggregated, results=FALSE, message=FALSE, warning=FALSE}
pt3_annexed = left_join(pt3_data, st_drop_geometry(hexagons_annexed), by = "GRID_ID") %>%
  st_drop_geometry()

pt3_annexed_aggregated = pt3_annexed %>%
  group_by(year, annex_category) %>%
  summarise(hom_ct = sum(hom_ct), bldg_ct = sum(bldg_ct)) %>%
  mutate(hom_rt = hom_ct / bldg_ct)

# Line Graph
ggplot(data = pt3_annexed_aggregated, aes(x = year, y = hom_rt, group = annex_category)) +
